...
def main():
    print("=== Training YOLOv8 Cheat Detector v5 ===")
    ...
    print("\n" + "="*60)
    print("TRAINING CONFIGURATION CONFIRMATION (v5):")
    print(f"  Epochs:         {epochs}")
    print(f"  Image Size:     {imgsz}")
    print(f"  Batch Size:     {batch}")
    print(f"  Device:         {device}")
    print(f"  Target Weight:  models/phone_chit_detector_v5.pt")
    ...
    model.train(
        ...
        device=device,
        project="runs",
        name="train_cheating_v5",
        exist_ok=False
    )
    
    # 5. Copy the best weights to models/phone_chit_detector_v5.pt
    best_weights_path = os.path.join(model.trainer.save_dir, "weights", "best.pt")
    target_weights_path = os.path.join(models_dir, "phone_chit_detector_v5.pt")
