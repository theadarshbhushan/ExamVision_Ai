...
V1_DIR = os.path.abspath("data/datasets/phone_chit_detection/exam_cheating_v1")
...
def remap_and_copy_v1_labels(src_dir, dst_dir):
    os.makedirs(dst_dir, exist_ok=True)
    for filename in os.listdir(src_dir):
        if not filename.endswith(".txt"):
            continue
        src_path = os.path.join(src_dir, filename)
        dst_path = os.path.join(dst_dir, f"v1_{filename}")
        ...
        for line in lines:
            ...
            # Remap class indices for exam_cheating_v1:
            # 0 (Looking around) -> 2 (peeking)
            # 1 (No cheating) -> convert to negative background (skip label)
            # 2 (Phone use) -> 3 (phone)
            if class_id == 0:
                class_id = 2  # peeking
            elif class_id == 1:
                continue      # negative background, skip
            elif class_id == 2:
                class_id = 3  # phone
            else:
                print(f"Warning: Unexpected class ID {class_id} in v1 file {src_path}")
                
            parts[0] = str(class_id)
            new_lines.append(" ".join(parts) + "\n")
            
        # Write the file (can be empty if all lines were skipped)
        with open(dst_path, "w", encoding="utf-8") as f:
            f.writelines(new_lines)

def copy_v1_images(src_dir, dst_dir):
    os.makedirs(dst_dir, exist_ok=True)
    for filename in os.listdir(src_dir):
        if not filename.lower().endswith((".jpg", ".jpeg", ".png")):
            continue
...
def main():
    ...
    # 4. Merge exam_cheating_v1 (train and valid splits only)
    print("\nMerging exam_cheating_v1 training data...")
    copy_v1_images(os.path.join(V1_DIR, "train", "images"), os.path.join(DST_DATASET_DIR, "train", "images"))
    remap_and_copy_v1_labels(os.path.join(V1_DIR, "train", "labels"), os.path.join(DST_DATASET_DIR, "train", "labels"))
    
    print("Merging exam_cheating_v1 validation data...")
    copy_v1_images(os.path.join(V1_DIR, "valid", "images"), os.path.join(DST_DATASET_DIR, "valid", "images"))
    remap_and_copy_v1_labels(os.path.join(V1_DIR, "valid", "labels"), os.path.join(DST_DATASET_DIR, "valid", "labels"))
